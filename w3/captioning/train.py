...
def train_test(dataloader, optimizer, model, criterion, mode="train"):
...
if __name__ == "__main__":
    ...
    for enc, dec, epochs, lr, opt, batch_size in zip(ENCODER, DECODER, EPOCHS, LEARNING_RATE, OPTIMIZER, BATCH_SIZE):
        ...
        NAME_MODEL=f"epoch{epochs}_lr{lr}_optimizer{opt}_encoder{enc}_decoder{dec}_{mapping}"
        wandb.init(entity=WANDB_ENTITY, project=WANDB_PROJECT, name=NAME_MODEL)
        ...
        early_stopping = EarlyStopping(save_path=f"{output_dir}/best_model.pth")

        for epoch in range(epochs):
            train_loss = train_test(train_loader, optimizer, model, criterion, mode="train")
            val_loss = train_test(val_loader, optimizer, model, criterion, mode="test")
